...
export async function backupToWebdav({
  ...
  const backupFileName = customFileName || `the-boss.${timestamp}.${hostname}.${deviceType}.zip`
...
export async function handleData(data: Record<string, any>) {
  if (data.version === 1) {
    ...
    localStorage.setItem('persist:the-boss', data.localStorage['persist:the-boss'])
