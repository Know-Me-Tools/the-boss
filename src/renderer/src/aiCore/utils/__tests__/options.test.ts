  ...
  describe('buildProviderOptions', () => {
    ...
    describe('Proxy provider custom parameters mapping', () => {
      it('should map cherryin provider ID to actual AI SDK provider ID (Google)', async () => {
        ...
        // User provides custom parameters with The Boss provider ID
        vi.mocked(getCustomParameters).mockReturnValue({
          cherryin: {
      ...
      ])('should route Gemini models to google providerOptions through $providerName', async ({
        providerId,
        providerName
      }) => {
        const { getCustomParameters } = await import('../reasoning')
      ...
      // instead of the The Boss provider ID for custom parameters to work correctly
      ...
      ])('should honor model.endpoint_type=$endpointType for $providerId and produce providerOptions.$expectedKey', async ({
        providerId,
        modelId,
        endpointType,
        expectedKey
