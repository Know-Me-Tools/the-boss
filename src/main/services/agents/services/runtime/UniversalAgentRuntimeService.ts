...
export interface UarProviderRuntimeOptions {
  ...
  modelId?: string
  /**
   * Selected model's endpoint type (e.g. 'openai', 'openai-response', 'anthropic').
   * Drives the UAR liter-llm transport protocol. When absent, the protocol is
   * inferred from the provider id (see resolveLiterLlmProtocol).
   */
  endpointType?: string
}

/**
 * UAR liter-llm transport protocol per provider: `chat` | `responses`.
 *
 * UAR's `auto` mode sniffs the backend and, for OpenAI-style `/v1` hosts, picks
 * the **Responses API** (`POST /v1/responses`, upgraded to a `wss://` stream).
 * Many OpenAI-*compatible* providers (e.g. MiniMax) only implement **chat
 * completions** and 404 on `/v1/responses`, which then loops on reconnect. So we
 * never emit `auto`: default to `chat` (the universal OpenAI-compatible shape) and
 * only use `responses` for providers explicitly known to support it (OpenAI, xAI).
 * A model's `endpoint_type` takes precedence when known.
 */
export function resolveLiterLlmProtocol(providerOptions: UarProviderRuntimeOptions): 'chat' | 'responses' {
  if (providerOptions.endpointType === 'openai-response') {
    return 'responses'
  }
  if (providerOptions.endpointType) {
    // Any other explicit endpoint type (openai, anthropic, gemini, …) is served
    // over the chat-completions-compatible shape by liter-llm.
    return 'chat'
  }

  const providerId = (providerOptions.providerId ?? '').toLowerCase()
  // Allowlist of providers whose default endpoint genuinely supports the OpenAI
  // Responses API. Keep this explicit and small.
  const RESPONSES_PROVIDER_IDS = new Set(['openai', 'xai'])
  return RESPONSES_PROVIDER_IDS.has(providerId) ? 'responses' : 'chat'
}
...
export function buildUarConfig({
  port,
  grpcPort,
  rocksDbPath,
  uploadsPath,
  ...
  const protocol = resolveLiterLlmProtocol(providerOptions)
